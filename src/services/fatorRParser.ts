...
export function extractFs12(lines: string[]) {
  // "Nenhuma" só conta dentro da seção 2.3) / 2.3.1)
  ...
  // Linhas que indicam tabela de tributos da página 2 — nunca devem ser FS12
  ...
  for (let i = 0; i < lines.length; i += 1) {
    ...
    // Preferir valor logo após o marcador "(R$)" na mesma linha
...
export function parsePgdasFatorR(rawText: string, fileName = ''): FatorRParseResult {
  ...
  // Sanidade: FS12 não pode ser maior que RBT12 (sempre indica captura errada na página 2)
