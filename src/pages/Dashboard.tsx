    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        <GlassCard variant="elevated" className="lg:col-span-2">
          ...
          <div className="space-y-2">
            {recent.length === 0 && <p className="rounded-xl border border-dashed border-border/70 p-9 text-center text-sm text-foreground/70">A pasta `a enviar` ainda não retornou guias.</p>}
            {recent.map((guide) => {
              ...
              return (
                <Link key={guide.id} to={`/guias/${guide.id}`} className="data-row">
                  <div className="min-w-0">
                    ...
                    <p className="mt-1 text-xs text-foreground/70">{company?.nomeFantasia || 'Identificando empresa'} | {formatDateTime(guide.receivedAt)}</p>
                  </div>
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <GlassCard>
          ...
          <div className="space-y-2">
            {openExceptions.length === 0 && <p className="py-8 text-center text-sm text-foreground/70">Nenhuma pendencia operacional.</p>}
          ...
          <div className="grid grid-cols-2 gap-2">
            {integrations.map((integration) => (
              <div key={integration.provider} className="rounded-xl border border-border/50 bg-card/40 p-3">
                <p className="text-xs text-foreground/70">{integration.displayName}</p>
                <p className="mt-2 text-sm font-semibold capitalize">{integration.status}</p>
              </div>
            ))}
            {integrations.length === 0 && <p className="col-span-2 py-8 text-center text-sm text-foreground/70">Conectores aguardando configuração.</p>}
          </div>
        </GlassCard>
      </div>
    </div>
