...
import { Bot, Play, Pause, RefreshCw, Zap, CheckCircle2, XCircle, AlertTriangle, Clock } from 'lucide-react';
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <PageHeader
        ...
        subtitle="Motor de coleta inteligente e monitoramento de conectores"
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
        <MetricCard label="Coletas Hoje" value={metrics.total} icon={<Zap className="h-4 w-4" />} />
        <MetricCard label="Sucesso" value={metrics.sucesso} icon={<CheckCircle2 className="h-4 w-4 text-success" />} variant="success" />
        <MetricCard label="Falhas" value={metrics.falha} icon={<XCircle className="h-4 w-4 text-destructive" />} variant={metrics.falha > 0 ? 'danger' : 'default'} />
        <MetricCard label="Revisão" value={metrics.revisao} icon={<AlertTriangle className="h-4 w-4 text-warning" />} variant={metrics.revisao > 0 ? 'warning' : 'default'} />
        <MetricCard label="Exceções" value={metrics.pendentes} icon={<AlertTriangle className="h-4 w-4" />} variant={metrics.pendentes > 0 ? 'danger' : 'default'} />
        <MetricCard label="Agendados" value={metrics.agendados} icon={<Clock className="h-4 w-4 text-info" />} />
      </div>
          ...
          {recentRuns.map(run => {
            ...
            return (
              <div
                key={run.id}
                className="data-row"
                onClick={() => navigate(`/execucoes/${run.id}`)}
              >
              ...
              {batch.status === 'executando' && (
                <div className="mt-2 h-1.5 rounded-full bg-muted overflow-hidden">
                  <div
                    ...
                    style={{ width: `${(batch.progressoAtual / batch.totalItems) * 100}%` }}
                  />
                </div>
              )}
            </div>
