...
export default function FatorR() {
  ...
  const load = async () => {
    ...
    setCompanies(c.data ?? []); setResults(r.data ?? []); setLogs(l.data ?? []); setSyncConfig(s.data ?? null);
  };
  ...
  const adjustResult = async (result: any) => {
    ...
    if (!reason) return;
    const raw = value.includes('%') ? Number(value.replace('%', '').replace(',', '.')) / 100 : Number(value.replace(',', '.'));
    ...
    await supabase.from('fator_r_monthly_results').update({ ...newData, metadata: { ...(result.metadata ?? {}), manual_review_recommended: false, updated_by_ui: true } }).eq('id', result.id);
    await supabase.from('fator_r_audit_logs').insert({ entity_type: 'fator_r_monthly_results', entity_id: result.id, action: 'manual_adjustment', old_data: oldData, new_data: newData, user_id: userData.user?.id ?? null });
    await load();
  };
  ...
  return <div className="space-y-5 animate-fade-in">
    <PageHeader title="Monitoramento de Fator R" subtitle="Acompanhamento automático dos extratos PGDAS e alertas preventivos por e-mail.">
      <div className="flex items-center gap-2">
        {folderUrl && <Button variant="outline" className="gap-1.5" onClick={() => window.open(folderUrl, '_blank')}><ExternalLink className="h-4 w-4" /> Abrir pasta no Drive</Button>}
        <Button onClick={runNow} className="gap-1.5" disabled={loadingSync}><FolderSync className="h-4 w-4" /> {loadingSync ? 'Processando...' : 'Rodar verificação agora'}</Button>
      </div>
    </PageHeader>

    <div className="grid grid-cols-2 lg:grid-cols-5 gap-3">
      {[
        ['Empresas', stats.monitored, 'from-violet-500/20 to-indigo-500/10'], ['Atenção', stats.attention, 'from-amber-500/25 to-orange-500/10'], ['Críticas', stats.critical, 'from-rose-500/25 to-red-500/10'], ['Seguras', stats.safe, 'from-emerald-500/25 to-green-500/10'], ['Resultados', results.length, 'from-sky-500/25 to-cyan-500/10'],
      ].map(([k,v,bg]) => <GlassCard key={String(k)} className={`p-3 rounded-2xl bg-gradient-to-br ${bg} border border-white/20 shadow-[inset_0_1px_0_rgba(255,255,255,0.35),0_12px_24px_-18px_rgba(0,0,0,.8)]`}>
    ...
    <div className="grid lg:grid-cols-2 gap-4">
      <GlassCard className="p-4 rounded-2xl">
        ...
        <div className="space-y-2 text-sm max-h-80 overflow-auto">
          {results.map((r) => <div key={r.id} className="flex items-center justify-between rounded-xl bg-muted/40 px-3 py-2 gap-2">
      ...
      <GlassCard className="p-4 rounded-2xl">
        ...
        <div className="space-y-2 text-sm">
          {logs.map((log) => <div key={log.id} className="flex justify-between rounded-xl bg-muted/40 px-3 py-2 gap-2"><span className="truncate">{log.event_type}: {log.message}</span><span className="text-xs text-foreground/60">{new Date(log.created_at).toLocaleString('pt-BR')}</span></div>)}
        </div>
      </GlassCard>
    </div>
  </div>;
