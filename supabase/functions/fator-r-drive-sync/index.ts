...
const pctToDecimal = (value: string) => { const n = value.trim().replace('%','').replace('.', '').replace(',', '.'); const raw = Number(n); return Number.isNaN(raw) ? null : (value.includes('%') || raw > 1 ? raw / 100 : raw); };
...
async function extractPdf(bytes: Uint8Array) {
  const pdf = await getDocumentProxy(bytes); const { text } = await extractText(pdf, { mergePages: true });
  ...
  try {
    ...
    for (const file of allFiles) {
      ...
      try {
        ...
        await supabase.from("fator_r_documents").update({ company_id: companyId, file_month: parsed.referenceMonth, file_year: parsed.referenceYear, detected_cnpj: parsed.cnpj, extraction_confidence: parsed.confidence, raw_text: rawText.slice(0, 20000), extracted_data: { ...parsed, source_folder_id: file.sourceFolderId }, processing_status: 'processed', processed_at: new Date().toISOString() }).eq("id", docIns.data.id);
        ...
        if (companyId && parsed.fatorRValue !== null && parsed.referenceMonth && parsed.referenceYear) {
          ...
          const status = parsed.fatorRValue <= Number(company.alert_threshold_critical) ? 'critical' : parsed.fatorRValue <= Number(company.alert_threshold_attention) ? 'attention' : 'safe';
          const recommendation = status === 'critical' ? 'Índice crítico: recomenda-se revisão imediata de pró-labore, folha e encargos.' : status === 'attention' ? 'Índice em atenção: recomenda-se revisão preventiva.' : 'Índice em faixa segura no cenário atual.';

          const upsert = await supabase.from("fator_r_monthly_results").upsert({ company_id: companyId, document_id: docIns.data.id, reference_month: parsed.referenceMonth, reference_year: parsed.referenceYear, fator_r_value: parsed.fatorRValue, fator_r_percent: parsed.fatorRPercent, status, recommendation, metadata: { confidence: parsed.confidence, source_folder_id: file.sourceFolderId } }, { onConflict: 'company_id,reference_month,reference_year' }).select("*").single();
          ...
          if (company.active && parsed.confidence >= 0.75 && status !== 'safe') {
            ...
            for (const recipient of recipients) {
              ...
              if (!alertErr && config?.email_alerts_enabled !== false) {
                ...
                await supabase.from("fator_r_alerts").update({ status: resp.error ? 'failed' : 'sent', sent_at: resp.error ? null : new Date().toISOString(), error_message: resp.error?.message ?? null }).eq("company_id", companyId).eq("monthly_result_id", upsert.data?.id).eq("alert_type", status).eq("recipient_email", recipient);
              }
            }
          }
        }

        await supabase.from("fator_r_processing_logs").insert({ document_id: docIns.data.id, company_id: companyId, event_type: 'file_processed', message: `Arquivo ${file.name} processado com sucesso.`, payload: { source_folder_id: file.sourceFolderId } });
        processed += 1;
      } catch (e) {
