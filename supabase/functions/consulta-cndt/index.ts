...
serve(async (req) => {
  ...
  try {
    ...
    const serproRes = await fetch(
      `https://gateway.apiserpro.serpro.gov.br/consulta-cndt/v2/cndt/${cnpjLimpo}`,
      { headers: { 'Authorization': `Bearer ${token}`, 'Accept': 'application/json' } }
    );

    const adminClient = createClient(Deno.env.get('SUPABASE_URL')!, Deno.env.get('SUPABASE_SERVICE_ROLE_KEY')!);
    const { data: connector } = await adminClient.from('connectors').select('id').eq('orgao', 'trabalhista').limit(1).single();
    ...
    if (!serproRes.ok) {
      if (connector) {
        await adminClient.from('connector_runs').insert({
          ...
          erro_detalhes: `Serpro CNDT retornou status ${serproRes.status}`,
        });
      }
      return new Response(JSON.stringify({ error: 'Erro na consulta CNDT', status: serproRes.status }), { status: 502, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
    }

    const resultado = await serproRes.json();
    const statusNorm = resultado.situacao?.toLowerCase().includes('nada consta') ? 'valida' : 'pendente';
    ...
    if (connector) {
      await adminClient.from('connector_runs').insert({
        ...
        evidencias: ['Resposta JSON da API Serpro - CNDT'],
      });
    }
